...
/// Set the thread-local arena pointer for tests. Must be called before using StyloNode.
fn init_arena(arena: &Arena) {
...
#[test]
fn stylo_node_size_is_usize() {
    assert_eq!(std::mem::size_of::<StyloNode>(), std::mem::size_of::<usize>());
}
...
fn stylo_node_new_and_access() {
    let arena = Arena::new();
    init_arena(&arena);
    let node = StyloNode::new(arena.document);
...
fn stylo_node_equality() {
    ...
    init_arena(&arena);
    let n1 = StyloNode::new(div1);
    let n2 = StyloNode::new(div2);
    let n1_copy = StyloNode::new(div1);
...
fn stylo_node_hash() {
    ...
    set.insert(StyloNode::new(div));
    set.insert(StyloNode::new(span));
    set.insert(StyloNode::new(div)); // duplicate
    assert_eq!(set.len(), 2);
}
...
fn node_info_is_element() {
    ...
    init_arena(&arena);
    assert!(StyloNode::new(div).is_element());
    assert!(!StyloNode::new(text).is_element());
    assert!(!StyloNode::new(arena.document).is_element());
}
...
fn node_info_is_text_node() {
    ...
    let div = make_element(&mut arena, "div");
    init_arena(&arena);
    assert!(StyloNode::new(text).is_text_node());
    assert!(!StyloNode::new(div).is_text_node());
    assert!(!StyloNode::new(arena.document).is_text_node());
}
...
fn tdocument_as_node_returns_self() {
    let arena = Arena::new();
    init_arena(&arena);
    let doc = StyloNode::new(arena.document);
...
fn tnode_parent_node() {
    ...
    init_arena(&arena);
    let html_node = StyloNode::new(html);
...
fn tnode_first_last_child() {
    ...
    let head_node = StyloNode::new(head);
...
fn tnode_siblings() {
    ...
    init_arena(&arena);
    let head_node = StyloNode::new(head);
    let body_node = StyloNode::new(body);
...
fn tnode_is_in_document() {
    ...
    init_arena(&arena2);
    assert!(!StyloNode::new(detached).is_in_document());
}
...
fn tnode_traversal_parent() {
    ...
    init_arena(&arena);
    let head_node = StyloNode::new(head);
    let parent = TNode::traversal_parent(&head_node).unwrap();
    assert_eq!(parent.id, html);

    // Root element's traversal parent is None (Document is not an element)
    let html_node = StyloNode::new(html);
    assert!(TNode::traversal_parent(&html_node).is_none());
}
...
fn sibling_element_navigation_skips_text() {
    ...
    let a_node = StyloNode::new(a);
    let b_node = StyloNode::new(b);
...
fn first_element_child_skips_text() {
    ...
    init_arena(&arena);
    assert_eq!(StyloNode::new(parent).first_element_child().unwrap().id, span);
}
...
fn first_element_child_none_when_only_text() {
    ...
    arena.append_child(parent, text);
    init_arena(&arena);
    assert!(StyloNode::new(parent).first_element_child().is_none());
}
...
fn parent_element_returns_none_at_root() {
    ...
    init_arena(&arena);
    let html_node = StyloNode::new(html);
    assert!(SelectorsElement::parent_element(&html_node).is_none());
}
...
fn parent_element_returns_parent_for_nested() {
    ...
    init_arena(&arena);
    let head_node = StyloNode::new(head);
    assert_eq!(SelectorsElement::parent_element(&head_node).unwrap().id, html);
}
...
fn is_root_for_html_element() {
    let (arena, html, head, _body) = build_simple_dom();
    init_arena(&arena);
    assert!(StyloNode::new(html).is_root());
    assert!(!StyloNode::new(head).is_root());
}
...
fn is_empty_with_no_children() {
    ...
    let div = make_element(&mut arena, "div");
    init_arena(&arena);
    assert!(StyloNode::new(div).is_empty());
}
...
fn is_link_anchor_with_href() {
    ...
    init_arena(&arena);
    assert!(StyloNode::new(a).is_link());
}
...
fn is_link_area_with_href() {
    ...
    init_arena(&arena);
    assert!(StyloNode::new(area).is_link());
}
...
fn is_link_includes_link_element() {
    ...
    init_arena(&arena);
    assert!(StyloNode::new(link).is_link());
}
...
fn is_link_div_is_false() {
    ...
    init_arena(&arena);
    assert!(!StyloNode::new(div).is_link());
}
...
fn attr_matches_exists() {
    ...
    assert!(node.attr_matches(&NamespaceConstraint::Any, &local, &AttrSelectorOperation::Exists));

    let missing = LocalName::from("data-y");
    assert!(!node.attr_matches(&NamespaceConstraint::Any, &missing, &AttrSelectorOperation::Exists));
}
...
fn match_pseudo_class_link_and_any_link() {
    ...
    init_arena(&arena);
    let node = StyloNode::new(a);

    use selectors::matching::{MatchingContext, MatchingMode, NeedsSelectorFlags, MatchingForInvalidation, SelectorCaches};
    let mut caches = SelectorCaches::default();
    let mut ctx = MatchingContext::<SelectorImpl>::new(
        MatchingMode::Normal, None, &mut caches,
        style::context::QuirksMode::NoQuirks,
        NeedsSelectorFlags::No, MatchingForInvalidation::No,
    );
...
fn is_html_element_in_html_document() {
    ...
    init_arena(&arena);
    assert!(StyloNode::new(div).is_html_element_in_html_document());
    assert!(!StyloNode::new(svg).is_html_element_in_html_document());
}
...
fn has_local_name() {
    ...
    init_arena(&arena);
    let node = StyloNode::new(div);
    assert!(node.has_local_name(&web_atoms::LocalName::from("div")));
    assert!(!node.has_local_name(&web_atoms::LocalName::from("span")));
}
...
fn has_namespace_html() {
    ...
    init_arena(&arena);
    let node = StyloNode::new(div);
    assert!(node.has_namespace(&web_atoms::Namespace::from("http://www.w3.org/1999/xhtml")));
    assert!(!node.has_namespace(&web_atoms::Namespace::from("http://www.w3.org/2000/svg")));
}
...
fn is_same_type() {
    ...
    let span = make_element(&mut arena, "span");
    init_arena(&arena);
    assert!(StyloNode::new(div1).is_same_type(&StyloNode::new(div2)));
    assert!(!StyloNode::new(div1).is_same_type(&StyloNode::new(span)));
}
...
fn selectors_opaque_different_for_different_nodes() {
    ...
    let b = make_element(&mut arena, "b");
    init_arena(&arena);
    let a_opaque = SelectorsElement::opaque(&StyloNode::new(a));
    let b_opaque = SelectorsElement::opaque(&StyloNode::new(b));
...
fn telement_as_node_returns_self() {
    ...
    let div = make_element(&mut arena, "div");
    init_arena(&arena);
    assert_eq!(TElement::as_node(&StyloNode::new(div)).id, div);
}
...
fn telement_traversal_children() {
    let (arena, html, head, body) = build_simple_dom();
    init_arena(&arena);
    let children: Vec<_> = StyloNode::new(html).traversal_children().map(|n| n.id).collect();
...
fn telement_traversal_children_empty() {
    ...
    let div = make_element(&mut arena, "div");
    init_arena(&arena);
    let children: Vec<_> = StyloNode::new(div).traversal_children().collect();
...
fn telement_is_html_svg_mathml() {
    ...
    assert!(StyloNode::new(div).is_html_element());
    assert!(!StyloNode::new(div).is_svg_element());
    assert!(StyloNode::new(svg).is_svg_element());
    assert!(StyloNode::new(math).is_mathml_element());
}
...
fn telement_state() {
    ...
    let div = make_element(&mut arena, "div");
    init_arena(&arena);
    assert_eq!(StyloNode::new(div).state(), ElementState::empty());

    arena.nodes[div].element_state = ElementState::HOVER;
    assert_eq!(StyloNode::new(div).state(), ElementState::HOVER);
}
...
fn telement_dirty_descendants() {
    ...
    let child_node = StyloNode::new(child);
    assert!(!child_node.has_dirty_descendants());

    unsafe { child_node.set_dirty_descendants() };
    assert!(child_node.has_dirty_descendants());
    assert!(StyloNode::new(parent).has_dirty_descendants());
...
fn telement_each_class() {
    ...
    StyloNode::new(div).each_class(|cls| classes.push(cls.to_string()));
...
fn telement_each_attr_name() {
    ...
    StyloNode::new(div).each_attr_name(|name| names.push(name.to_string()));
...
fn telement_is_html_document_body_element() {
    let (arena, _html, head, body) = build_simple_dom();
    init_arena(&arena);
    assert!(StyloNode::new(body).is_html_document_body_element());
    assert!(!StyloNode::new(head).is_html_document_body_element());
}
...
fn telement_no_animations() {
    ...
    let div = make_element(&mut arena, "div");
    init_arena(&arena);
    assert!(!StyloNode::new(div).may_have_animations());
}
...
fn telement_local_name_and_namespace() {
    ...
    // prepare_for_stylo caches the atoms that local_name/namespace return
    crate::css::resolve::resolve_styles(&mut arena);
    init_arena(&arena);
    let node = StyloNode::new(div);
    assert_eq!(&**node.local_name(), "div");
    assert_eq!(&**node.namespace(), "http://www.w3.org/1999/xhtml");
}
...
fn telement_selector_flags() {
    ...
    assert!(node.relative_selector_search_direction().is_empty());
}
...
fn attribute_provider_get_attr() {
    ...
    init_arena(&arena);
    let node = StyloNode::new(div);
    assert_eq!(node.get_attr(&LocalName::from("id")).as_deref(), Some("test"));
    assert_eq!(node.get_attr(&LocalName::from("class")).as_deref(), Some("foo"));
    assert!(node.get_attr(&LocalName::from("data-x")).is_none());
}
...
fn attribute_provider_non_element_returns_none() {
    ...
    let text = make_text(&mut arena, "hello");
    init_arena(&arena);
    assert!(StyloNode::new(text).get_attr(&LocalName::from("anything")).is_none());
}

// ─── parse_size_attr ──────────────────────────────────────────────────────

#[test]
fn parse_size_attr_plain_number_is_px() {
    use style::values::specified::{LengthPercentage, NoCalcLength, AbsoluteLength};
    let result = parse_size_attr("100").unwrap();
    match result {
        LengthPercentage::Length(NoCalcLength::Absolute(AbsoluteLength::Px(v))) => {
            assert_eq!(v, 100.0);
        }
...
fn parse_size_attr_px_suffix() {
    use style::values::specified::{LengthPercentage, NoCalcLength, AbsoluteLength};
    let result = parse_size_attr("200px").unwrap();
    match result {
        LengthPercentage::Length(NoCalcLength::Absolute(AbsoluteLength::Px(v))) => {
            assert_eq!(v, 200.0);
        }
...
fn parse_size_attr_percentage() {
    ...
    let result = parse_size_attr("50%").unwrap();
    match result {
        LengthPercentage::Percentage(p) => {
            assert!((p.0 - 0.5).abs() < 0.001, "expected 0.5, got {}", p.0);
        }
        other => panic!("expected Percentage(0.5), got {:?}", other),
    }
}
...
fn parse_size_attr_zero() {
    use style::values::specified::{LengthPercentage, NoCalcLength, AbsoluteLength};
    let result = parse_size_attr("0").unwrap();
    match result {
        LengthPercentage::Length(NoCalcLength::Absolute(AbsoluteLength::Px(v))) => {
            assert_eq!(v, 0.0);
        }
...
fn parse_size_attr_invalid() {
    ...
    assert!(parse_size_attr("px").is_none());
    assert!(parse_size_attr("%").is_none());
}

#[test]
fn parse_size_attr_negative_rejected() {
    assert!(parse_size_attr("-10").is_none());
    assert!(parse_size_attr("-1px").is_none());
}

#[test]
fn parse_size_attr_float() {
    let result = parse_size_attr("12.5");
...
fn presentational_hints_hidden_attribute() {
    ...
    StyloNode::new(div).synthesize_presentational_hints_for_legacy_attributes(
        VisitedHandlingMode::AllLinksUnvisited, &mut hints,
    );
...
fn presentational_hints_width_height_on_img() {
    ...
    init_arena(&arena);
    let mut hints: Vec<ApplicableDeclarationBlock> = Vec::new();
    StyloNode::new(img).synthesize_presentational_hints_for_legacy_attributes(
        VisitedHandlingMode::AllLinksUnvisited, &mut hints,
    );
...
fn presentational_hints_width_on_non_supported_element() {
    ...
    init_arena(&arena);
    let mut hints: Vec<ApplicableDeclarationBlock> = Vec::new();
    StyloNode::new(p).synthesize_presentational_hints_for_legacy_attributes(
        VisitedHandlingMode::AllLinksUnvisited, &mut hints,
    );
    assert!(hints.is_empty());
}

// ─── Cached atom tests (no Box::leak) ─────────────────────────────────────

#[test]
fn cached_atoms_populated_by_prepare_for_stylo() {
    let mut arena = Arena::new();
    let div = make_element_with_attrs(&mut arena, "div", &[("id", "main")]);
    arena.append_child(arena.document, div);

    // Before prepare_for_stylo: caches are None
    assert!(arena.nodes[div].cached_atom_local_name.is_none());
    assert!(arena.nodes[div].cached_atom_namespace.is_none());
    assert!(arena.nodes[div].cached_atom_id.is_none());

    // Run prepare_for_stylo (called by resolve_styles)
    crate::css::resolve::resolve_styles(&mut arena);

    // After: caches are populated
    assert!(arena.nodes[div].cached_atom_local_name.is_some());
    assert!(arena.nodes[div].cached_atom_namespace.is_some());
    assert!(arena.nodes[div].cached_atom_id.is_some());

    let local = arena.nodes[div].cached_atom_local_name.as_ref().unwrap();
    assert_eq!(&**local, "div");

    let ns = arena.nodes[div].cached_atom_namespace.as_ref().unwrap();
    assert_eq!(&**ns, "http://www.w3.org/1999/xhtml");

    let id = arena.nodes[div].cached_atom_id.as_ref().unwrap();
    assert_eq!(&**id, "main");
}

#[test]
fn cached_atoms_used_by_telement_methods() {
    let mut arena = Arena::new();
    let div = make_element_with_attrs(&mut arena, "div", &[("id", "test")]);
    arena.append_child(arena.document, div);
    crate::css::resolve::resolve_styles(&mut arena);
    ...
    // local_name() should return a pointer into the cached field, not a leaked box
    let local_name_ptr = node.local_name() as *const web_atoms::LocalName;
    let cached_ptr = arena.nodes[div]
        .cached_atom_local_name
        .as_ref()
        .unwrap() as *const web_atoms::LocalName;
    assert_eq!(
        local_name_ptr, cached_ptr,
        "local_name() should return pointer to cached atom, not a leaked allocation"
    );

    // namespace() should return a pointer into the cached field
    let ns_ptr = node.namespace() as *const web_atoms::Namespace;
    let cached_ns_ptr = arena.nodes[div]
        .cached_atom_namespace
        .as_ref()
        .unwrap() as *const web_atoms::Namespace;
    assert_eq!(
        ns_ptr, cached_ns_ptr,
        "namespace() should return pointer to cached atom, not a leaked allocation"
    );

    // id() should return a pointer into the cached field
    let id_ptr = node.id().unwrap() as *const Atom;
    let cached_id_ptr = arena.nodes[div]
        .cached_atom_id
        .as_ref()
        .unwrap() as *const Atom;
    assert_eq!(
        id_ptr, cached_id_ptr,
        "id() should return pointer to cached atom, not a leaked allocation"
    );
}

#[test]
fn repeated_resolve_does_not_leak() {
    // Run resolve_styles 100 times on the same arena.
    // If Box::leak were used, this would leak ~100 * num_elements * 3 atoms.
    // With caching, each resolve overwrites the same cached fields — no growth.
    let mut arena = crate::dom::parse_document(
        "<html><head></head><body><div id='a'><span id='b'>text</span></div></body></html>",
    );

    for _ in 0..100 {
        // Clear previous stylo data so resolve can run again
        for node in arena.nodes.values_mut() {
            *node.stylo_element_data.get_mut() = None;
            node.dirty_descendants = std::sync::atomic::AtomicBool::new(false);
        }
        crate::css::resolve::resolve_styles(&mut arena);
    }

    // If we get here without OOM, the caching works.
    // Verify the caches are still valid after 100 runs.
    let div = arena.find_element(arena.document, "div").unwrap();
    assert_eq!(
        &**arena.nodes[div].cached_atom_local_name.as_ref().unwrap(),
        "div"
    );
    assert_eq!(
        &**arena.nodes[div].cached_atom_id.as_ref().unwrap(),
        "a"
    );
}

#[test]
fn no_id_element_has_no_cached_id() {
    let mut arena = Arena::new();
    let div = make_element(&mut arena, "div"); // no id attribute
    arena.append_child(arena.document, div);
    crate::css::resolve::resolve_styles(&mut arena);
